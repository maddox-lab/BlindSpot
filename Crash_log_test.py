...
import shutil
from BlindSpot3_2 import build_journal_state, run_blinder, track_action, read_journal, recover_from_journal, where_file_go
import pytest
...
def test_track_function(tmp_path): 
    ...
    with open(log_file, newline="", encoding="utf-8") as f:
        ...
        assert rows[0]["Old file path"] == "old.jpeg"
        assert rows[0]["Destination path"] == "DEST"
...
def test_read_journal(tmp_path): 
    ...
    assert rows[1]["Old file path"] == "old-2.jpeg"
    assert rows[1]["Destination path"] == "DEST-2"
...
def test_abort_reconcile(tmp_path):
    ...
    assert (finalized, aborted, lost) == (0, 1, 0)
    assert_log_has_action(log_file, "aborted", str(old))  
...
def test_done_reconcile(tmp_path):
    ...
    assert_log_has_action(log_file, "done", str(old))  
...
def test_lost_reconcile_journal(tmp_path):
    ...
    assert_log_has_action(log_file, "lost", str(old))  
...
def test_pending_reconcile_journal(tmp_path):
    ...
    assert str(old) in journal_state
    assert journal_state[str(old)]["done"] is False
...
def test_crash_log(tmp_path):
    ...
    assert journal_state[key]["pending"]["dest"] == str(dest)
...
def test_reconcile_multi(tmp_path):
    ...
    track_action(log_file, TxtID="1", old_path=str(old1), dest_path=str(dest1),
                   old_name=old1.name, new_name=dest1.name, action="start", message="m")
    
    track_action(log_file, TxtID="2", old_path=str(old2), dest_path=str(dest2),
                     old_name=old2.name, new_name=dest2.name, action="start", message="m")
...
def test_crash_copy(tmp_path, monkeypatch):
    ...
    assert copy_calls["count"] == num_run # make sure the correct number of copies exist
    ...
    assert len(list(tmp_path.glob("img_*.nd2"))) == num_files # checks the originals should still exist because clone_og=True
...
def test_rerun(tmp_path, monkeypatch):
    ...
    def crash_replace(src, dst):
        ...
        if count["n"] == num_run: # simulate crash on the rename/replace step after num_run files have been processed
