...
def build_journal_state(log_path): # Make sure the actions are saved in the place thaat the files exist
    """
...
def where_file_go(
    ...
    txtid = uuid.uuid4().hex[:10]  # creates a unique txtID to link through the files so in case something goes whack with the name
    ...
    **kwargs,  # makes tests resilient to extra args
    ...
    try:
        ...
        for fpath in files:
            ...
            except Exception as e: # Exception to make sure the program doesn't crash if one file is weird
            ...
            with open(map_path, "w", newline="", encoding="utf-8") as f:
                ...
                w.writerow(
                    [
                        "Old file name",
                        "New file name",
                        "Relative Path from chosen folder",
                    ]
                )

                for old_full, new_name in final_mapping.items():
                    old_full_p = Path(old_full)
                    rel_parent = safe_path(chosen_base, old_full_p.parent)
                    w.writerow([old_full_p.name, new_name, rel_parent])
