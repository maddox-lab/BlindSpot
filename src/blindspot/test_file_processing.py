...
from  .functions.BlindSpot_core import gen_name, safe_path
...
def test_path_outside(tmp_path):
    ...
    assert output
    assert (
        output.startswith("..") or output.startswith("../") or output.startswith("..\\")
    )  # Checks if the output is outside base directory
